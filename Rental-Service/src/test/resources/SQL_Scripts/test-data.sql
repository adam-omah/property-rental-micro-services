...
drop sequence if exists rentals_seq;
...
INSERT INTO tenant (tenant_id, tenant_name, tenant_email, tenant_phone)
SELECT nextval('TENANT_SEQ'), 'Julius Ceasar', 'j.ceasar@example.com', '087-123-4567'
    WHERE NOT EXISTS (SELECT 1 FROM tenant WHERE tenant_email = 'j.ceasar@example.com');
